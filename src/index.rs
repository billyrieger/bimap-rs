//! A bimap backed by two `IndexMap`s.
...
pub struct BiIndexMap<L, R, LS = hash_map::RandomState, RS = hash_map::RandomState> {
    left2right: IndexMap<Ref<L>, Ref<R>, LS>,
    right2left: IndexMap<Ref<R>, Ref<L>, RS>,
}

impl<L, R> BiIndexMap<L, R, hash_map::RandomState, hash_map::RandomState>
where
    L: Eq + Hash,
    R: Eq + Hash,
{
    /// Creates an empty `BiIndexMap`.
    ///
    /// # Examples
    ///
    /// ```
    /// use bimap::BiIndexMap;
    ///
    /// let bimap = BiIndexMap::<char, i32>::new();
    /// ```
    pub fn new() -> Self {
        Self {
            left2right: IndexMap::new(),
            right2left: IndexMap::new(),
        }
    }

    /// Creates a new empty `BiIndexMap` with the given capacity.
    ///
    /// # Examples
    ///
    /// ```
    /// use bimap::BiIndexMap;
    ///
    /// let bimap = BiIndexMap::<char, i32>::with_capacity(10);
    /// assert!(bimap.capacity() >= 10);
    /// ```
    pub fn with_capacity(capacity: usize) -> Self {
        Self {
            left2right: IndexMap::with_capacity(capacity),
            right2left: IndexMap::with_capacity(capacity),
        }
    }
}

impl<L, R, LS, RS> BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
{
    ...
    /// Returns a lower bound on the number of left-right pairs the `BiIndexMap`
...
where
    ...
    RS: BuildHasher,
{
    /// Creates a new empty `BiIndexMap` using `hash_builder_left` to hash left
    ...
    /// let mut bimap = BiIndexMap::<char, i32>::with_hashers(s_left, s_right);
    /// bimap.insert('a', 42);
    /// ```
    pub fn with_hashers(hash_builder_left: LS, hash_builder_right: RS) -> Self {
        Self {
            left2right: IndexMap::with_hasher(hash_builder_left),
            right2left: IndexMap::with_hasher(hash_builder_right),
        }
    }

    /// Creates a new empty `BiIndexMap` with the given capacity, using
    ...
    /// let s_left = RandomState::new();
    /// let s_right = RandomState::new();
    /// let bimap = BiIndexMap::<char, i32>::with_capacity_and_hashers(10, s_left, s_right);
    /// assert!(bimap.capacity() >= 10);
    /// ```
    pub fn with_capacity_and_hashers(
        ...
        Self {
            left2right: IndexMap::with_capacity_and_hasher(capacity, hash_builder_left),
            right2left: IndexMap::with_capacity_and_hasher(capacity, hash_builder_right),
        }
    }

    /// Reserves capacity for at least `additional` more elements to be inserted
    /// in the `BiIndexMap`. The collection may reserve more space to avoid
    ...
    /// use bimap::BiIndexMap;
    ///
    /// let mut bimap = BiIndexMap::<char, i32>::new();
    /// bimap.reserve(10);
    /// assert!(bimap.capacity() >= 10);
    /// ```
    ...
    /// use bimap::BiIndexMap;
    ///
    /// let mut bimap = BiIndexMap::<char, i32>::with_capacity(100);
    /// bimap.insert('a', 1);
    /// bimap.insert('b', 2);
    ...
    ///
    /// ```
    /// use bimap::{BiIndexMap, Overwritten};
    ///
    /// let mut bimap = BiIndexMap::new();
...
impl<L, R, LS, RS> Clone for BiIndexMap<L, R, LS, RS>
where
    L: Clone + Eq + Hash,
    R: Clone + Eq + Hash,
...
{
    fn clone(&self) -> BiIndexMap<L, R, LS, RS> {
        let mut new_bimap = BiIndexMap::with_capacity_and_hashers(
            self.capacity(),
...
impl<L, R, LS, RS> fmt::Debug for BiIndexMap<L, R, LS, RS>
where
    L: fmt::Debug,
    R: fmt::Debug,
{
...
impl<L, R, LS, RS> Default for BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
...
{
    fn default() -> BiIndexMap<L, R, LS, RS> {
        BiIndexMap {
            left2right: IndexMap::default(),
            right2left: IndexMap::default(),
        }
    }
}

impl<L, R, LS, RS> Eq for BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
    LS: BuildHasher,
...
impl<L, R, LS, RS> FromIterator<(L, R)> for BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
    LS: BuildHasher + Default,
    RS: BuildHasher + Default,
{
    fn from_iter<I>(iter: I) -> BiIndexMap<L, R, LS, RS>
    where
        I: IntoIterator<Item = (L, R)>,
    {
        ...
        let mut bimap = match iter.size_hint() {
            (lower, None) => {
                BiIndexMap::with_capacity_and_hashers(lower, LS::default(), RS::default())
            }
            (_, Some(upper)) => {
                BiIndexMap::with_capacity_and_hashers(upper, LS::default(), RS::default())
            }
        };
...
impl<'a, L, R, LS, RS> IntoIterator for &'a BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
{
...
impl<L, R, LS, RS> IntoIterator for BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
{
...
impl<L, R, LS, RS> Extend<(L, R)> for BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
    LS: BuildHasher,
...
impl<L, R, LS, RS> PartialEq for BiIndexMap<L, R, LS, RS>
where
    L: Eq + Hash,
    R: Eq + Hash,
    LS: BuildHasher,
...
/// An owning iterator over the left-right pairs in a `BiIndexMap`.
pub struct IntoIter<L, R> {
    inner: map::IntoIter<Ref<L>, Ref<R>>,
}
...
/// An iterator over the left-right pairs in a `BiIndexMap`.
///
/// This struct is created by the [`iter`] method of `BiIndexMap`.
///
/// [`iter`]: BiIndexMap::iter
#[derive(Debug, Clone)]
pub struct Iter<'a, L, R> {
    inner: map::Iter<'a, Ref<L>, Ref<R>>,
}
...
/// An iterator over the left values in a `BiIndexMap`.
///
/// This struct is created by the [`left_values`] method of `BiIndexMap`.
///
/// [`left_values`]: BiIndexMap::left_values
#[derive(Debug, Clone)]
...
/// An iterator over the right values in a `BiIndexMap`.
///
/// This struct is created by the [`right_values`] method of `BiIndexMap`.
///
/// [`right_values`]: BiIndexMap::right_values
#[derive(Debug, Clone)]
pub struct RightValues<'a, L, R> {
    inner: map::Iter<'a, Ref<R>, Ref<L>>,
}
...
unsafe impl<L, R, LS, RS> Send for BiIndexMap<L, R, LS, RS>
where
    L: Send,
    R: Send,
    LS: Send,
...
unsafe impl<L, R, LS, RS> Sync for BiIndexMap<L, R, LS, RS>
where
    L: Sync,
    R: Sync,
    LS: Sync,
...
mod tests {
    ...
    #[test]
    fn default() {
        let _ = BiIndexMap::<char, i32>::default();
    }
    ...
    #[test]
    fn from_iter() {
        let bimap = BiIndexMap::from_iter(vec![
            ('a', 1),
            ('b', 2),
            ('c', 3),
            ('b', 2),
    ...
    #[test]
    fn clear() {
        let mut bimap = vec![('a', 1)].into_iter().collect::<BiIndexMap<_, _>>();
    ...
    #[test]
    fn get_contains() {
        let bimap = vec![('a', 1)].into_iter().collect::<BiIndexMap<_, _>>();
